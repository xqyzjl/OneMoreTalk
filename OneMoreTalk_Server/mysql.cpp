...
{
    QString sqlselect = "SELECT friendId"
    ...
    QSqlQuery sqlqueryt = *sqlquery;
    while(sqlqueryt.next()) {
        friendinfo.append(selectUserPublicInfo(sqlqueryt.value(0).toInt()));
    }
    return friendinfo;
}
...
QList<int> MySql::selectUserGroupInfo(int userid)
{
    ...
    while(sqlquery->next()) {
        groupinfo.append(-1*sqlquery->value(0).toInt());
    }
    return groupinfo;
}
...
QList<PublicInfo> MySql::selectGroupInfo(int groupid)
{
    ...
    QSqlQuery sqlqueryt = *sqlquery;
    while(sqlqueryt.next()) {
        groupinfo.append(selectUserPublicInfo(sqlqueryt.value(0).toInt()));
    }
    return groupinfo;
}
...
int MySql::selectMaxId()
{
    QString sqlselect = "SELECT MAX(userId)"
                        " FROM usersInfo";
    int maxId = 1000;
    if(sqlquery->prepare(sqlselect)) {
        if(sqlquery->exec(sqlselect)) {
